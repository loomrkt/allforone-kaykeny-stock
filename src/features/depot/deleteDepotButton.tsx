...
function DeleteDepotButton({ depot }: { depot: Depot }) {
  ...
  const mutation = useMutation({
    ...
    onError: () => {
      ...
      toastUtils.toast({
        title: "Échec de la suppression du dépôt.",
        description: new Date().toLocaleDateString(),
        action: <ToastAction altText="Fermer">Fermer</ToastAction>,
      });
    },
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{`Supprimer le dépôt ${depot.name}`}</DialogTitle>
          <DialogDescription>
...
export default DeleteDepotButton;
